class CanvasCursorBlueprint {
   ...
   * Set a cursor
   *
   * @param {string} handler – returns handler name as argument
   */
  set(name, canvas) {
    let customCursors = {
    ...
    if (typeof customCursors[name] === "undefined" && defaultCursors.indexOf(name) === -1) {
      ...
      canvas.style.cursor = customCursors[name] || name;
    }
  }
}


window.CanvasCursor = new CanvasCursorBlueprint();

export let CanvasCursor = window.CanvasCursor;